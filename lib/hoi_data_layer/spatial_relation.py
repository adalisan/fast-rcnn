...
def get_map_no_pad(box1, box2, length):
    ...
    bx1_rs = np.array( \
    	[int(round(bx1[0] * factor_w)), int(round(bx1[1] * factor_h)),
         int(round(bx1[2] * factor_w)), int(round(bx1[3] * factor_h))])
    bx2_rs = np.array( \
    	[int(round(bx2[0] * factor_w)), int(round(bx2[1] * factor_h)),
         int(round(bx2[2] * factor_w)), int(round(bx2[3] * factor_h))])
    # generate map
    map_1 = np.zeros([length, length], dtype=np.uint8)
    map_2 = np.zeros([length, length], dtype=np.uint8)
    for i in range(bx1_rs[1], bx1_rs[3]):
        for j in range(bx1_rs[0], bx1_rs[2]):
            map_1[i][j] = 1
    for i in range(bx2_rs[1], bx2_rs[3]):
        for j in range(bx2_rs[0], bx2_rs[2]):
            map_2[i][j] = 1
    return np.array([map_1, map_2])
...
def get_map_pad(box1, box2, length):
    ...
    if h > w:
        ...
        bx1_rs = np.array([int(round(bx1[0] * factor) + num_pad),
        	               int(round(bx1[1] * factor)),
        	               int(round(bx1[2] * factor) + num_pad),
        	               int(round(bx1[3] * factor))])
        bx2_rs = np.array([int(round(bx2[0] * factor) + num_pad),
        	               int(round(bx2[1] * factor)),
        	               int(round(bx2[2] * factor) + num_pad),
        	               int(round(bx2[3] * factor))])
    else:
        factor = np.float(length) / np.float(w)
        num_pad = int(round((length - factor * h) / 2.0))
        bx1_rs = np.array([int(round(bx1[0] * factor)),
        	               int(round(bx1[1] * factor) + num_pad),
                           int(round(bx1[2] * factor)),
                           int(round(bx1[3] * factor) + num_pad)])
        bx2_rs = np.array([int(round(bx2[0] * factor)),
        	               int(round(bx2[1] * factor) + num_pad),
                           int(round(bx2[2] * factor)),
                           int(round(bx2[3] * factor) + num_pad)])
    # generate map
    map_1 = np.zeros([length, length], dtype=np.uint8)
    map_2 = np.zeros([length, length], dtype=np.uint8)
    for i in range(bx1_rs[1], bx1_rs[3]):
